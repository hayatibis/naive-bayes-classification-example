...
count_vect = CountVectorizer(analyzer='word', ngram_range=(1, 3), min_df=0.007, max_df=0.4)
...
for file in os.listdir("KDataset/"):
    f = open("KDataset/" + file, encoding='utf-8')
    context = f.read()
...
output = open("kaggle.txt", "w")

output.write("InstanceName,Class\n")

for i in range(len(LabelResult)):
    output.write(str(testFileNames[i]) + ',' + str(LabelResult[i]) + '\n')
