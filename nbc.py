...
count_vect = CountVectorizer(analyzer='word', ngram_range=(1, 2), min_df=0.0005, max_df=0.4)
...
train_V = train_V.toarray()

n_word_pos = np.sum(train_V[:positiveCount], axis=0)
n_word_neg = np.sum(train_V[positiveCount:], axis=0)

total_word_pos = np.sum(n_word_pos)
total_word_neg = np.sum(n_word_neg)

pos_prob_vector = np.log1p((n_word_pos + 1) / (total_word_pos + len(features)))
neg_prob_vector = np.log1p((n_word_neg + 1) / (total_word_neg + len(features)))
...
for i in range(len(LabelResult)):
    output.write(
        str(testFileNames[i]) + ' TrueLabel :' + str(testLabels[i]) + ' EstimatedLabel :' + str(LabelResult[i]) + '\n')
